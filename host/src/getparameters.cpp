             ...
             )
{
  if (mypars->dpffile)
  {
    ...
    std::string line;
    char tempstr[256], argstr[256];char  argstr2[256];
    char* args[2];
    int tempint, i, len;
    float tempfloat;
    int line_count = 0;
    ...
    char **ltypes     = NULL;
    char **tmp_ltypes = NULL;
    std::string map_fn;
    int idx;
    pair_mod* curr_pair;
    float paramA, paramB;
    int m, n;
    char typeA[7], typeB[7];
      ...
      switch(token_id){
        case DPF_MOVE: // movable ligand file name
            if(!mypars->xml2dlg)
            {
              ...
              {
                printf("ERROR: Bug? The initial keyword of ligand file name should be ligand or move\n.");
                exit(1);
              }
              ...
              mypars->ligandfile = strdup(line.c_str());
            }
            break;
            ...
            for(i=strlen(tempstr); i<(int)line.size(); i++){
              if(isspace(line[i])){ // whitespace
                ...
                if(len<0){ // new type starts
                  ...
                  ltype_nr++;
                  ltype_li++;
                  tmp_ltypes = (char **) realloc(ltypes, ltype_nr * sizeof(char *));
                  if(tmp_ltypes == NULL)
                  {
                    printf("ERROR: Cannot allocate memory for tmp_ltypes.\n");
                    exit(1);
                  }
                  if(tmp_ltypes != ltypes)
                  {
                    ltypes = tmp_ltypes;
                  }
                  tmp_ltypes = NULL;
                  ltypes[ltype_li] = (char *) calloc(7, 1);
                }
                if(i-len<6){
                  ltypes[ltype_li][i-len] = line[i];
                } else{
                  printf("\nError: Atom types are limited to 6 characters in <%s> parameter at %s:%u.\n",tempstr,mypars->dpffile,line_count);
                  return 1;
                }
              }
            }
        ...
        case DPF_MAP: // grid map specifier
            ...
            n=-1;
            for(long unsigned int m=mygrid->grid_mapping.size()/2; m<mygrid->grid_mapping.size(); m++)
            ...
            if(strcmp(argstr,ltypes[mtype_nr])){ // derived type
              ...
              if(idx == 0){
                printf("Error: Derivative (ligand type %s) names can only be upto 6 characters long.\n",ltypes[mtype_nr]);
                return 1;
              }
              if(idx>0){
        ...
        case DPF_GALS: // actually run a search (only if xml2dlg isn't specified)
            if(!mypars->xml2dlg){
              ...
              {
                printf("Error: Value of <%s> at %s:%u must be an integer larger than 0.\n",tempstr,mypars->dpffile,line_count);
                return -1;
              }
        ...
        case GA_pop_size: // population size
            ...
            {
              printf("Error: Value of <%s> at %s:%u must be an integer larger than 1.\n",tempstr,mypars->dpffile,line_count);
              return 1;
            }
            break;
        case GA_num_generations: // number of generations
            sscanf(line.c_str(),"%*s %s", tempNumStr);
            mpz_set_str(mypars->num_of_generations, tempNumStr, 10);
            if(mpz_cmp_si(mypars->num_of_generations, 0) <= 0)
            {
              printf("Error: Value of <%s> at %s:%u must be larger than 0.\n",tempNumStr, mypars->dpffile,line_count);
              return 1;
            }
            break;
        case GA_num_evals: // number of evals
            sscanf(line.c_str(),"%*s %s", tempNumStr);
            mpz_set_str(mypars->num_of_energy_evals, tempNumStr, 10);
            if(mpz_cmp_si(mypars->num_of_energy_evals, 0) > 0)
            {
        ...
        case DPF_COMMENT: // we use comments to allow specifying AD-GPU command lines
            ...
            if(tempstr[0]=='-'){ // potential command line argument
              ...
              if(argcmp("devnum",tempstr,'D')){
                new_device=true;
                for(long unsigned int i=0; i<mypars->dev_pool.size(); i++){
...
// run is stored separately to allow logical parsing with the correct parameters initialized per run
{
  ...
  mpz_init_set_ui(mypars->num_of_energy_evals, 2500000);
  mpz_init_set_ui(mypars->num_of_generations, 42000);
  mpz_init_set_ui(mypars->heuristics_max, 12000000);
  mpz_init(mypars->initial_sw_generations);
#ifdef TOOLMODE
  mypars->xml2dlg = true;
                  ...
                  )
{
  ...
  printf("--stateFileInName      | Name for the input docking state file.                | stateIn.bin\n");
#endif
  printf("\nOUTPUT\n");
  printf("--resnam            -N | Prefix for docking output                             | ligand basename\n");
  ...
  printf("--gbest                | Number of output pdbqt files of the best pose of each | 0\n");
  printf("                       | run (up to the number of run)                         |\n");
  printf("--miniCoorOut          | The coordinates in the minimization stage will be     | 0 (no)\n");
  printf("                       | written to standard output if the value is not zero.  |\n");
  printf("                       | If this value is not equal to 0, then the following   |\n");
  printf("                       | is set: --psize 2 --nrun 1 --heurmax 10.              |\n");
  printf("--stateFileOutPre      | Prefix for the output docking state file.             | stateOut\n");
  printf("--showTopRunNum        | The number of top runs of that the energy be showed.  | 0\n");
  printf("--showTopIdvNum        | The number of top individuals of that the energy be   | 0\n");
  printf("                       | showed.                                               |\n");
  printf("--cautionOn            | Show the caution information.                         | 0 (no)\n");
  printf("--warningOn            | Show the warning information.                         | 0 (no)\n");
  ...
  printf("--inputConfRatio       | The ratio of the population that the conformation     | 0.2\n");
  printf("                       | genes are initialized as file input.                  |\n");
  ...
  printf("--ligaStepSize         | The step size for ligand only minimization.           | 0.1\n");
  printf("--compStepSize         | The step size for complex minimization.               | 0.1\n");
  printf("--ligaRhoLimit         | The maximal rho value for ligand minimization. Larger | 100\n");
  printf("                       | rho, more steps for ligand minimization.              |\n");
  printf("--compRhoLimit         | The maximal rho value for complex minimization.       | 1000\n");
  printf("                       | Larger rho, more steps for complex minimization.      |\n");
  printf("--collRhoLimit         | The maximal rho value for collision-eliminating       | 5\n");
  printf("                       | minimization. Larger rho, more steps for              |\n");
  printf("                       | collision-eliminating minimization.                   |\n");
  printf("--collDistance         | The distance at which non-bonded atoms are considered | 1.3\n");
  printf("                       | to have collided (exclude H-H).                       |\n");
  printf("--rebornRatio          | The ratio of population that is treated as bad        | 0.2\n");
  printf("                       | samples and will be regenerated.                      |\n");
  printf("--miniOnly             | Do minimization only. If this value is not equal to   | 0 (no)\n");
  printf("                       | 0, then the following is set: --miniCoorOut 1.        |\n");
  printf("--addonEng             | The amount of energy that needs to be added to set    | 400\n");
  printf("                       | the threshold of low energy.                          |\n");
  printf("--lowEngNumRatio       | The maximal ratio of the low energy number in the     | 0.1\n");
  printf("                       | population.                                           |\n");
  printf("--convergedThre        | The threshold of energy difference to define the      | 0.01\n");
  printf("                       | convergence of the energy used for comparison.        |\n");
  printf("--ligandOnly           | Find the lowest energy conformation of ligand alone.  | 0 (no)\n");
#endif
  printf("\nSCORING\n");
  ...
  printf("--savetime             | Do not re-calculate final energy with CPU as the the  | 0 (no)\n");
  printf("                       | number of run is more than one. (0/1)                 |\n");
  printf("--topN4rescore         | The number of top individuals of that the energy be   | 1\n");
  printf("                       | re-calculated as the number of run is one.            |\n");
  printf("--enthalpyScaling      | The value for enthalpy scaling.                       | 0.7564\n");
  printf("--entropyScaling       | The value for entropy scaling.                        | 0.9\n");
  printf("--shakingRadius        | The radius of atom shaking for calculate entropy (in  | 0.75\n");
  printf("                       | Å).                                                   |\n");
  printf("--wellDepth            | The depth of a energy landscape for calculating       | 0.35\n");
  printf("                       | entropy (in kcal/mol).                                |\n");
#ifndef TOOLMODE
  printf("\nOmegaDocker requires a ligand and a set of grid maps to perform a docking calculation. Optionally,\n");
...
// the data in the same format as it is required for writing it to algorithm defined registers.
{
...
#endif
  {
    ...
    if (argcmp("nev", argv[i], 'e'))
    {
      arg_recognized = 1;
//       long tempLong;
//       sscanf(argv[i+1], "%ld", &tempLong);
//       if ((tempLong > 0) && (tempLong < 0x7FFFFFFFFFFFFFFF)){
//         mypars->num_of_energy_evals = (unsigned long) tempLong;
      char tempNumStr[1024];
      sscanf(argv[i+1],"%s", tempNumStr);
      mpz_set_str(mypars->num_of_energy_evals, tempNumStr, 10);
      if(mpz_cmp_si(mypars->num_of_energy_evals, 0) > 0)
      {
        mypars->nev_provided = true;
      } else{
        printf("Error: Value of --nev (-e) argument must be between 0 and 2^63-1.\n");
        return -1;
      }
    }
    ...
    if (argcmp("ngen", argv[i], 'g'))
    {
      ...
      if(mpz_cmp_si(mypars->num_of_generations, 0) <= 0)
      {
        printf("Error: Value of --ngen (-g) argument must be larger then 0.\n");
        return -1;
      }
    }
    ...
    if (argcmp("initswgens", argv[i]))
    {
      ...
      char tempNumStr[1024];
      sscanf(argv[i+1], "%s", tempNumStr);
      mpz_set_str(mypars->initial_sw_generations, tempNumStr, 10);
      if(mpz_cmp_si(mypars->initial_sw_generations, 0) <= 0)
      {
        printf("Error: Value of --initswgens argument must be larger then 0.\n");
        return -1;
      }
    }
    ...
    if (argcmp("heurmax", argv[i], 'E'))
    {
      ...
      char tempNumStr[1024];
      sscanf(argv[i+1], "%s", tempNumStr);
      mpz_set_str(mypars->heuristics_max, tempNumStr, 10);
      if(mpz_cmp_si(mypars->heuristics_max, 0) <= 0)
      {
        printf("Error: Value of --heurmax (-E) argument must be larger then 0.\n");
        return -1;
      }
      if(mypars->miniCoorOut || mypars->miniOnly)
      {
        mpz_set_ui(mypars->heuristics_max, 10);
      }
    }
    ...
    if (argcmp("psize", argv [i], 'p'))
    {
      ...
      {
        printf("Error: Value of --psize (-p) argument must be an integer larger than 1.\n");
        return -1;
      }
      break;
    }
    ...
    if (argcmp("npdb", argv [i]))
    {
      ...
      if (tempint < 0)
      {
        printf("Error: Value of --npdb argument must be an integer larger than 0.\n");
        return -1;
      }
      else
      {
    ...
    if (argcmp("nrun", argv [i], 'n'))
    {
      ...
      {
        printf("Error: Value of --nrun (-n) argument must be an integer larger than 0.\n");
        return -1;
      }
    }
    ...
    if (argcmp("gbest", argv [i]))
    {
      ...
      {
        mypars->gen_best = tempint;
      }
    }

    // Argument: skip time consuming result processing without change coordinates of the best one
    // If the value is equal to or less than zero, no result processing will be skipped.
    if (argcmp("savetime", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint <= 0)
      {
        mypars->savetime = false;
      }
      else
      {
        mypars->savetime = true;
      }
    }

    if (argcmp("inputConfRatio", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      {
        mypars->inputConfRatio = tempfloat;
      }
      else
      {
        printf("Error: Value of --inputConfRatio argument must be a float between 0 and 1 or equal to 0 or 1.\n");
        return -1;
      }
    }

    if (argcmp("miniCoorOut", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      {
        mypars->miniCoorOut = false;
      }
    }

    if (argcmp("ligaStepSize", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat > 0.0f)
      {
        mypars->ligaStepSize = tempfloat;
      }
      else
      {
        printf("Error: Value of --ligaStepSize argument must be a float larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("compStepSize", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat > 0.0f)
      {
        mypars->compStepSize = tempfloat;
      }
      else
      {
        printf("Error: Value of --compStepSize argument must be a float larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("ligaRhoLimit", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint > 0)
      {
        mypars->ligaRhoLimit = tempint;
      }
      else
      {
        printf("Error: Value of --ligaRhoLimit argument must be larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("compRhoLimit", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint > 0)
      {
        mypars->compRhoLimit = tempint;
      }
      else
      {
        printf("Error: Value of --compRhoLimit argument must be larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("collRhoLimit", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint > 0)
      {
        mypars->collRhoLimit = tempint;
      }
      else
      {
        printf("Error: Value of --collRhoLimit argument must be larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("collDistance", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat > 0.0f)
      {
        mypars->collDistance = tempfloat;
      }
      else
      {
        printf("Error: Value of --collDistance argument must be larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("rebornRatio", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);

      if (tempfloat >= 0.0f && tempfloat <= 0.9f)
      {
        mypars->rebornRatio = tempfloat;

        tempfloat = 1 - mypars->lowEngNumRatio;
        if(mypars->rebornRatio > tempfloat)
        {
          mypars->rebornRatio = tempfloat;
        }
      }
      ...
      {
        printf("Error: Value of --rebornRatio argument must be larger than or equal to 0 and also less than or equal to 0.9.\n");
        return -1;
      }
    }

    if (argcmp("miniOnly", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
    ...
    if (argcmp("addonEng", argv [i]))
    {
      ...
      if (tempfloat > 0.0f)
      {
        mypars->addonEng = tempfloat;
      }
      else
      {
        printf("Error: Value of --addonEng argument must be larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("lowEngNumRatio", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat >= 0.0f && tempfloat <= 0.9f)
      {
        mypars->lowEngNumRatio = tempfloat;
      ...
      {
        printf("Error: Value of --lowEngNumRatio argument must be larger than or equal to 0 and less than or equal to 0.9.\n");
        return -1;
      }
    }

    if (argcmp("convergedThre", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat > 0.0f)
      {
        mypars->convergedThre = tempfloat;
      }
      else
      {
        printf("Error: Value of --convergedThre argument must be larger than 0.\n");
        return -1;
      }
    }

    if (argcmp("stateFileInName", argv [i]))
    {
      arg_recognized = 1;
      strncpy(mypars->stateFileInName, argv[i+1], 512);
      mypars->stateFileInName[511] = '\0';
      mypars->readState = true;
    }

    if (argcmp("stateFileOutPre", argv [i]))
    {
      arg_recognized = 1;
      strncpy(mypars->stateFileOutPre, argv[i+1], 512);
      mypars->stateFileOutPre[511] = '\0';
      mypars->writeState = true;
    }

    if (argcmp("showTopRunNum", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);

      if (tempint > mypars->num_of_runs)
      {
        mypars->showTopRunNum = mypars->num_of_runs;
      }
      else if (tempint > 0)
      {
        mypars->showTopRunNum = tempint;
      }
    }

    if (argcmp("showTopIdvNum", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);

      if (tempint > mypars->pop_size)
      {
        mypars->showTopIdvNum = mypars->pop_size;
      }
      else if (tempint > 0)
      {
        mypars->showTopIdvNum = tempint;
      }
    }
    
    if (argcmp("topN4rescore", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint > mypars->pop_size)
      {
        mypars->topN4rescore = mypars->pop_size;
      }
      else if (tempint > 0)
      {
        mypars->topN4rescore = tempint;
      }
    }
    
    if (argcmp("enthalpyScaling", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat >= 0.0f)
      {
        mypars->enthalpyScaling = tempfloat;
      }
      else
      {
        printf("Error: Value of --enthalpyScaling argument must be larger than or equal to 0.\n");
        return -1;
      }
    }
    
    if (argcmp("entropyScaling", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat >= 0.0f)
      {
        mypars->entropyScaling = tempfloat;
      }
      else
      {
        printf("Error: Value of --entropyScaling argument must be larger than or equal to 0.\n");
        return -1;
      }
    }
    
    if (argcmp("shakingRadius", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat > 0.0f)
      {
        mypars->shakingRadius = tempfloat;
      }
      else
      {
        printf("Error: Value of --shakingRadius argument must be larger than 0.\n");
        return -1;
      }
    }
    
    if (argcmp("wellDepth", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%f", &tempfloat);
      ...
      if (tempfloat > 0.0f)
      {
        mypars->wellDepth = tempfloat;
      }
      else
      {
        printf("Error: Value of --wellDepth argument must be larger than 0.\n");
        return -1;
      }
    }
    
    if (argcmp("cautionOn", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint <= 0)
      {
        mypars->cautionOn = false;
      }
      else
      {
        mypars->cautionOn = true;
      }
    }
    
    if (argcmp("warningOn", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint <= 0)
      {
        mypars->warningOn = false;
      }
      else
      {
        mypars->warningOn = true;
      }
    }
    
    if (argcmp("ligandOnly", argv [i]))
    {
      arg_recognized = 1;
      sscanf(argv [i+1], "%d", &tempint);
      ...
      if (tempint <= 0)
      {
        mypars->ligandOnly = false;
      }
      else
      {
        mypars->ligandOnly = true;
      }
    }
                       ...
                       )
{
  ...
  if(error){
    printf("Error: XML file is not in OmegaDocker format (error #%d in line %lu).\n",error,line_nr);
    exit(error);
  }
}

float **           read_xml_genomes(
                                    char* xml_filename,
                                    float grid_spacing,
                                    int &nrot,
                                    Liganddata *myligand_init,
                                    long unsigned int *runNum,
                                    bool store_axisangle
                                   )
{
  float **result    = NULL;
  float **resultTmp = NULL;
  std::ifstream file(xml_filename);
  if(file.fail()){
    printf("\nError: Could not open xml file %s. Check path and permissions.\n", xml_filename);
    exit(3);
  ...
  while(std::getline(file, line)) {
    ...
    if(line.find("<run id=")==0){
      ...
      if(run_nr>count){
        resultTmp = (float **) realloc(result, run_nr * sizeof(float *));
        if(resultTmp == NULL)
        {
          printf("ERROR: Cannot allocate memory for parsing xml file information.\n");
          exit(1);
        }
        if(resultTmp != result)
        {
        ...
        resultTmp = NULL;
        for(int i = count; i < run_nr; i++)
        {
          result[i] = (float *) malloc(genotype_length_in_globmem * sizeof(float));
        }
        count=run_nr;
      }
    }
    ...
    if(line.find("<axisangle0>")==0){
      if(run_nr>0){
        ...
        if(!store_axisangle){
          ...
          *(gene+2) = genrot;
        } else result[run_nr-1][genotype_length_in_globmem-1] = genrot;
        found_genome++;
      } else{
        error=10;
        break;
  ...
  *runNum = (long unsigned int) count;
  return result;
}
...
// Each contiguous GENOTYPE_LENGTH_IN_GLOBMEM pieces of floats in init_population corresponds to a genotype
{
  ...
  int genotype_length_in_globmem = myligand->num_of_rotbonds + 7;
  char *tempNumStr;
  ...
  long unsigned int read_run_num = 0;
  printf("    Name for the input docking state file  : %s\n", mypars->stateFileInName);
  
  printf("    Prefix for the docking output          : %s\n", mypars->resname);
  if(mypars->contact_analysis)
  {
    printf("    (Yes) Perform distance-based analysis.\n");
  }
  else
  {
    printf("    (No) Do not perform distance-based analysis.\n");
  }
  if(mypars->output_xml)
  {
    printf("    (Yes) Xml output is enabled.\n");
  }
  else
  {
    printf("    (No) Xml output is disabled.\n");
  }
  if(mypars->output_dlg)
  {
    printf("    (Yes) Dlg output is enabled.\n");
  }
  else
  {
    printf("    (No) Dlg output is disabled.\n");
  }
  if(mypars->dlg2stdout)
  {
    printf("    (Yes) Dlg output to stdout is enabled.\n");
  }
  else
  {
    printf("    (No) Dlg output to stdout is disabled.\n");
  }
  if(mypars->reflig_en_required)
  {
    printf("    (Yes) Print reference ligand energies.\n");
  }
  else
  {
    printf("    (No) Do not print reference ligand energies.\n");
  }
  if(mypars->gen_finalpop)
  {
    printf("    (Yes) Output all poses from all populations of each LGA run.\n");
  }
  else
  {
    printf("    (No) Do not output all poses from all populations of each LGA run.\n");
  }
  printf("    # pose pdbqt files from populations of each LGA run: %d\n", mypars->gen_pdbs);
  printf("    Number of best poses will be write to pdbqt        : %d\n", mypars->gen_best);
  if(mypars->miniCoorOut)
  {
    printf("    (Yes) The coordinates in the minimization stage will be written to standard output.\n");
  }
  else
  {
    printf("    (No) The coordinates in the minimization stage will not be written to standard output.\n");
  }
  printf("    Prefix for the output docking state file              : %s\n", mypars->stateFileOutPre);
  printf("    Number of top runs of that the energy be showed       : %d\n", mypars->showTopRunNum);
  printf("    Number of top individuals of that the energy be showed: %d\n", mypars->showTopIdvNum);
  if(mypars->cautionOn)
  {
    printf("    (Yes) Show the caution information.\n");
  }
  else
  {
    printf("    (No) Do not show the caution information.\n");
  }
  if(mypars->warningOn)
  {
    printf("    (Yes) Show the warning information.\n");
  }
  else
  {
    printf("    (No) Do not show the warning information.\n");
  }
  if(mypars->calc_clustering)
  {
    printf("    (Yes) Output clustering analysis in dlg and/or xml file.\n");
  }
  else
  {
    printf("    (No) Do not output clustering analysis in dlg and/or xml file.\n");
  }
  if(mypars->handle_symmetry)
  {
    printf("    (Yes) Handle symmetry in RMSD calculation.\n");
  }
  else
  {
    printf("    (No) Do not Handle symmetry in RMSD calculation.\n");
  }
  printf("    RMSD clustering tolerance                            : %f Å\n", mypars->rmsd_tolerance);
  
  // printf("    Cuda device number (counting starts at 1): %d\n", mypars->devnum + 1);
  if(mypars->load_xml)
  {
    printf("    Load initial population from xml results file        : %s\n", mypars->load_xml);
  }
  else
  {
    printf("    Do not load initial population from xml results file.\n");
  }
  printf("    Random number seeds (up to three comma-sep. integers): %d, %d, %d\n", mypars->seed[0], mypars->seed[1], mypars->seed[2]);
  
  if(mypars->use_heuristics)
  {
    printf("    (Yes) Use heuristics\n");
  }
  else
  {
    printf("    (No) Do not Use heuristics.\n");
  }
  tempNumStr = mpz_get_str(NULL, 10, mypars->heuristics_max);
  printf("    Evals limit (smooth limit) of asymptotic heuristics: %s\n", tempNumStr);
  free(tempNumStr);
  if(mypars->autostop)
  {
    printf("    (Yes) Automatic stopping criterion based on convergence is enabled.\n");
  }
  else
  {
    printf("    (No) Automatic stopping criterion based on convergence is disabled.\n");
  }
  printf("    AutoStop testing frequency (in # of generations): %d\n", mypars->as_frequency);
  printf("    # LGA runs                                      : %d\n", mypars->num_of_runs);
  printf("    Local-search method                             : %s\n", mypars->ls_method);
  printf("    # Local-search iterations (max.)                : %u\n", mypars->max_num_of_iters);
  printf("    (popSize) Number of population size             : %d\n", pop_size);
  printf("    Ratio of conformations as input                 : %f\n", mypars->inputConfRatio);
  int inputConformationNum = (int) (((float) pop_size) * mypars->inputConfRatio);
  printf("    Number of conformations as input                : %d\n", inputConformationNum);
  printf("    Mutation rate                                   : %f %\n", mypars->mutation_rate);
  printf("    Crossover rate                                  : %f %\n", mypars->crossover_rate);
  printf("    The step size for ligand only minimization      : %f\n", mypars->ligaStepSize);
  printf("    The step size for complex minimization          : %f\n", mypars->compStepSize);
  printf("    The maximal rho value for ligand minimization   : %d\n", mypars->ligaRhoLimit);
  printf("    The maximal rho value for complex minimization  : %d\n", mypars->compRhoLimit);
  printf("    The maximal rho value for collision-eliminating minimization                             : %d\n", mypars->collRhoLimit);
  printf("    The distance at which non-bonded atoms are considered to have collided (exclude H-H)     : %f Å\n", mypars->collDistance);
  printf("    The ratio of population that is treated as bad samples and will be regenerated           : %f\n", mypars->rebornRatio);
  int alivePopNum = (int) ((1.0f - mypars->rebornRatio) * mypars->pop_size);
  printf("    (alivePopNum) Number of population that is treated as bad samples and will be regenerated: %d\n", mypars->pop_size - alivePopNum);
  if(mypars->miniOnly)
  {
    printf("    (Yes) Do minimization only.\n");
  }
  else
  {
    printf("    (No) Not minimization only.\n");
  }
  printf("    The amount of energy that needs to be added to set the threshold of low energy                : %f kcal/mol\n", mypars->addonEng);
  printf("    The maximal ratio of the low energy number in the population                                  : %f\n", mypars->lowEngNumRatio);
  int maxReservedIdvNum = (int) (mypars->lowEngNumRatio * mypars->pop_size);
  printf("    (maxReservedIdvNum) Number of the low energy number in the population                         : %d\n", maxReservedIdvNum);
  printf("    The threshold of energy difference to define the convergence of the energy used for comparison: %f kcal/mol\n", mypars->convergedThre);
  
  if(mypars->savetime)
  {
    printf("    (Yes) Re-calculate part of final energies with CPU.\n");
  }
  else
  {
    printf("    (No) Re-calculate all final energies with CPU\n");
  }
  if(mypars->topN4rescore > maxReservedIdvNum)
  {
    mypars->topN4rescore = maxReservedIdvNum;
  }
  printf("    (topN4rescore) The number of top individuals of that the energy be re-calculated as the number of run is one: %d\n", mypars->topN4rescore);
  printf("    About the population:\n");
  printf("      |             gbest|      topN4rescore| maxReservedIdvNum|       alivePopNum|           popSize|\n");
  printf("     0|%18d|%18d|%18d|%18d|%18d|\n", mypars->gen_best, mypars->topN4rescore, maxReservedIdvNum, alivePopNum, mypars->pop_size);
  printf("    The value for enthalpy scaling                         : %f\n", mypars->enthalpyScaling);
  printf("    The value for entropy scaling                          : %f\n", mypars->entropyScaling);
  printf("    The radius of atom shaking for calculate entropy       : %f Å\n", mypars->shakingRadius);
  printf("    The depth of a energy landscape for calculating entropy: %f kcal/mol\n", mypars->wellDepth);
  
  
  if(mypars->load_xml){ // read population data from previously output xml file
    int nrot;
    float **genome = read_xml_genomes(mypars->load_xml, mygrid->spacing, nrot, myligand, &read_run_num);
    ...
    if(read_run_num < mypars->num_of_runs)
    {
      printf("Note: XML contains %ld genomes but %lu runs are requested, randomizing other runs.\n",read_run_num, mypars->num_of_runs);
    }
    else if(read_run_num > mypars->num_of_runs)
    {
      printf("Note: XML contains %ld genomes but %lu runs are requested, use only %lu runs.\n",read_run_num, mypars->num_of_runs, mypars->num_of_runs);
      read_run_num = mypars->num_of_runs;
    }
    for (long unsigned int run_id = 0; run_id < read_run_num; run_id++)
    {
      memcpy(init_populations[run_id], genome[run_id], sizeof(float) * genotype_length_in_globmem);
      for(int pop_id = 1; pop_id < pop_size; pop_id++)
      {
        // randomize location and convert to grid coordinates
        for(gene_id = 0; gene_id < 3; gene_id++)
        {
          init_populations[run_id][pop_id * genotype_length_in_globmem + gene_id] = r.random_float() * (mygrid->size_xyz_angstr[gene_id]) / mygrid->spacing;
        }
        ...
        init_populations[run_id][pop_id*genotype_length_in_globmem + 3] = phi      / DEG_TO_RAD;
        init_populations[run_id][pop_id*genotype_length_in_globmem + 4] = theta    / DEG_TO_RAD;
        init_populations[run_id][pop_id*genotype_length_in_globmem + 5] = rotangle / DEG_TO_RAD;
        for(int gene_id = 6; gene_id < myligand->num_of_rotbonds + 6; gene_id++)
        {
          init_populations[run_id][pop_id*genotype_length_in_globmem + gene_id] = r.random_float() * 360;
        }
      }
    }
  }
  for(long unsigned int run_id = read_run_num; run_id < mypars->num_of_runs; run_id++)
  {
    for(int pop_id = 0; pop_id < inputConformationNum; pop_id++)
    {
      // randomize location and convert to grid coordinates
      for(gene_id = 0; gene_id < 3; gene_id++)
      {
      ...
      for(int gene_id = 6; gene_id < myligand->num_of_rotbonds + 6; gene_id++)
      {
        init_populations[run_id][pop_id*genotype_length_in_globmem + gene_id] = 0;
      }
    }
    for(int pop_id = inputConformationNum; pop_id < pop_size; pop_id++)
    {
      // randomize location and convert to grid coordinates
      for(gene_id = 0; gene_id < 3; gene_id++)
      {
