...
//                        the row index is equal to the atom type code.
  char         **base_atom_types;
// atom_map_to_fgrids:    Maps each moving atom to a (pre-loaded) map id
  int           *atom_map_to_fgrids;
// ligand grid types
  char         **ligand_grid_types;
...
struct IntraTables{
  ...
  {
    q1q2 = (float **) malloc(myligand->num_of_atoms * sizeof(float *));
    if(q1q2 == NULL)
    {
      printf("ERROR: Cannot allocate memory for q1q2.\n");
      exit(1);
    }
    for(int i = 0; i < myligand->num_of_atoms; i++)
    {
      q1q2[i] = (float *) malloc(myligand->num_of_atoms * sizeof(float));
      if(q1q2[i] == NULL)
      {
        printf("ERROR: Cannot allocate memory for q1q2[i].\n");
        exit(1);
      }
    }
    
    qasp_mul_absq = (float *) malloc(myligand->num_of_atoms * sizeof(float));
    if(qasp_mul_absq == NULL)
    {
      printf("ERROR: Cannot allocate memory for qasp_mul_absq.\n");
      exit(1);
    }
    
    is_HB = (bool **) malloc(myligand->num_of_atypes * sizeof(bool *));
    if(is_HB == NULL)
    {
      printf("ERROR: Cannot allocate memory for is_HB.\n");
      exit(1);
    }
    for(int i = 0; i < myligand->num_of_atypes; i++)
    {
      is_HB[i] = (bool *) malloc(myligand->num_of_atypes * sizeof(bool));
      if(is_HB[i] == NULL)
      {
        printf("ERROR: Cannot allocate memory for is_HB[i].\n");
        exit(1);
      }
    }
    
    mod_pair = (pair_mod ***) malloc(myligand->num_of_atypes * sizeof(pair_mod **));
    if(mod_pair == NULL)
    {
      printf("ERROR: Cannot allocate memory for mod_pair.\n");
      exit(1);
    }
    for(int i = 0; i < myligand->num_of_atypes; i++)
    {
      mod_pair[i] = (pair_mod **) malloc(myligand->num_of_atypes * sizeof(pair_mod *));
      if(mod_pair[i] == NULL)
      {
        printf("ERROR: Cannot allocate memory for mod_pair[i].\n");
        exit(1);
      }
    }
